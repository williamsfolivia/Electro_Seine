...
SpeciesRichness<-CountFish %>%
  group_by(FIELDID,Method,DateSampled) %>% dplyr::summarise(across(american_brook_lamprey:yellow_perch, sum)) %>% 
...
```{r}
Final_metrics<-Final_P51cal %>% select(FIELDID,Method,DateSampled,TotalFish,Richness,DarterCount,SunfishCount,SuckerCount,IntolerantCount,Percent_Tolerant,Percent_Omnivores,Percent_Piscivores,Percent_Lithophilic, Percent_Insectivores)
...
","main12","md-14")))  %>%  dplyr::filter(month=="6"|month=="7"|month=="8"|month=="9")
...
write.csv(Entire_combined, "Data/Created_Data/Entire_Combined_modern.csv")
